...
use crate::graph::cache::AdjacencyMap;
use crate::graph::sqlite_graph::NodeId;
...
pub struct SnapshotState {
    ...
    pub outgoing: HashMap<NodeId, AdjacencyMap>,
    /// Immutable copy of incoming adjacency map  
    pub incoming: HashMap<NodeId, AdjacencyMap>,
...
impl SnapshotState {
    ...
    pub fn new(
        outgoing: &HashMap<NodeId, AdjacencyMap>,
        incoming: &HashMap<NodeId, AdjacencyMap>,
    ) -> Self {
        Self {
    ...
    pub fn get_outgoing(&self, node_id: NodeId) -> Option<&AdjacencyMap> {
        self.outgoing.get(&node_id)
    }

    /// Get incoming neighbors for a node in this snapshot
    pub fn get_incoming(&self, node_id: NodeId) -> Option<&AdjacencyMap> {
...
impl SnapshotManager {
    ...
    pub fn new() -> Self {
        let initial_state = Arc::new(SnapshotState::new(&HashMap::new(), &HashMap::new()));
        Self {
            current: ArcSwap::new(initial_state),
        }
    }
    ...
    pub fn with_state(
        ...
        let initial_state = Arc::new(SnapshotState::new(outgoing, incoming));
        Self {
            current: ArcSwap::new(initial_state),
        }
    }
    ...
    pub fn update_snapshot(
        ...
        let new_state = Arc::new(SnapshotState::new(outgoing, incoming));
        self.current.store(new_state);
    }
...
/// Read-only snapshot view for querying graph data
/// 
/// Provides safe, read-only access to snapshot data with its own
/// SQLite connection to ensure database consistency.
...
mod tests {
    ...
    fn test_snapshot_state_creation() {
        ...
        outgoing.insert(1, adj.clone());
        incoming.insert(1, adj);
    ...
    fn test_snapshot_manager() {
        ...
        // Update state
        outgoing.insert(2, AdjacencyMap::new());
        incoming.insert(2, AdjacencyMap::new());
