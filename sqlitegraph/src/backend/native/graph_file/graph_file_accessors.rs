...
use crate::backend::native::{
    ...
    types::{NodeRecord, EdgeRecord, NativeNodeId, NativeEdgeId},
...
impl GraphFile {
    ...
    pub fn read_node_at(&mut self, node_id: NativeNodeId) -> crate::backend::native::types::NativeResult<NodeRecord> {
        NodeEdgeAccessManager::read_node_at(self, node_id)
    }

    /// Write node record at specific node ID
    pub fn write_node_at(&mut self, node_id: NativeNodeId, node: &NodeRecord) -> crate::backend::native::types::NativeResult<()> {
        use crate::backend::native::constants::node::NODE_SLOT_SIZE;

        if node_id == 0 {
            return Err(crate::backend::native::types::NativeBackendError::InvalidNodeId {
                id: node_id,
                max_id: 0
            });
        }
    ...
    pub fn read_edge_at_offset(&mut self, offset: u64) -> crate::backend::native::types::NativeResult<EdgeRecord> {
        NodeEdgeAccessManager::read_edge_at_offset(self, offset)
    }

    /// Read edge record at specific edge ID
    pub fn read_edge_at(&mut self, edge_id: NativeEdgeId) -> crate::backend::native::types::NativeResult<EdgeRecord> {
        let offset = self.calculate_edge_offset(edge_id);
        self.read_edge_at_offset(offset)
    }

    /// Write edge record at specific edge ID
    pub fn write_edge_at(&mut self, edge_id: NativeEdgeId, edge: &EdgeRecord) -> crate::backend::native::types::NativeResult<()> {
        NodeEdgeAccessManager::write_edge_at(self, edge_id, edge)
    }

    /// Check if node exists
    pub fn node_exists(&mut self, node_id: NativeNodeId) -> crate::backend::native::types::NativeResult<bool> {
        NodeEdgeAccessManager::node_exists(self, node_id)
    }

    /// Get node statistics
    pub fn get_node_statistics(&self) -> crate::backend::native::types::NativeResult<crate::backend::native::graph_file::header::ClusterUtilization> {
        HeaderManager::get_node_statistics(&self.persistent_header)
    }

    /// Get edge statistics
    pub fn get_edge_statistics(&self) -> crate::backend::native::types::NativeResult<crate::backend::native::graph_file::header::ClusterUtilization> {
    ...
    pub fn get_node_record_size(&self, node: &NodeRecord) -> crate::backend::native::types::NativeResult<usize> {
        NodeEdgeAccessManager::get_node_record_size(node)
    }

    /// Get edge record size for serialization
    pub fn get_edge_record_size(&self, edge: &EdgeRecord) -> crate::backend::native::types::NativeResult<usize> {
    ...
    /// Reserve node slots
    pub fn reserve_node_slots(&mut self, count: u32) -> crate::backend::native::types::NativeResult<()> {
        NodeEdgeAccessManager::reserve_node_slots(self, count)
    }

    /// Reserve edge slots
    pub fn reserve_edge_slots(&mut self, count: u32) -> crate::backend::native::types::NativeResult<()> {
