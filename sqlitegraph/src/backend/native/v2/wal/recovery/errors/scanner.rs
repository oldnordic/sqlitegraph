...
impl ScannerErrorContext {
    ...
    pub fn wal_file_read(
        ...
        context
            .metadata
            .insert("bytes_requested".to_string(), bytes_requested.to_string());

        if let Some(read) = bytes_read {
            context
                .metadata
                .insert("bytes_read".to_string(), read.to_string());
        }
    ...
    pub fn wal_file_parse(
        ...
        if let Some(data) = actual_data {
            context
                .metadata
                .insert("data_length".to_string(), data.len().to_string());
            if data.len() <= 16 {
                context
                    .metadata
                    .insert("data_preview".to_string(), format!("{:?}", data));
            }
        }
    ...
    pub fn wal_header_validation(
        ...
        context
            .metadata
            .insert("header_size".to_string(), header_size.to_string());
        context
            .metadata
            .insert("expected_version".to_string(), expected_version.to_string());

        if let Some(version) = actual_version {
            context
                .metadata
                .insert("actual_version".to_string(), version.to_string());
        }
    ...
    pub fn wal_index_scan(
        ...
        context
            .metadata
            .insert("index_offset".to_string(), index_offset.to_string());
        context
            .metadata
        ...
        if let Some(entry) = failed_at_entry {
            context
                .metadata
                .insert("failed_at_entry".to_string(), entry.to_string());
    ...
    pub fn wal_sequence_scan(
        ...
        context
            .metadata
            .insert("start_lsn".to_string(), start_lsn.to_string());
        context
            .metadata
            .insert("end_lsn".to_string(), end_lsn.to_string());
        context.records_processed = Some(records_scanned);

        if let Some(lsn) = current_lsn {
            context
                .metadata
                .insert("current_lsn".to_string(), lsn.to_string());
        }
    ...
    pub fn file_system_access(
        ...
        if let Some(perm) = permissions_required {
            context
                .metadata
                .insert("permissions_required".to_string(), perm.to_string());
        }
    ...
    pub fn format_detection(
        ...
        if let Some(bytes) = magic_bytes {
            context
                .metadata
                .insert("magic_bytes".to_string(), format!("{:?}", bytes));
        }

        if let Some(format) = suspected_format {
            context
                .metadata
                .insert("suspected_format".to_string(), format.to_string());
        }
    ...
    pub fn buffer_management(
        ...
        context
            .metadata
            .insert("buffer_size".to_string(), buffer_size.to_string());
        context
            .metadata
            .insert("required_size".to_string(), required_size.to_string());
...
impl ScannerErrorFactory {
    ...
    pub fn wal_read_error(
        ...
        RecoveryError::new(RecoveryErrorKind::Io, message)
            .with_context(ScannerErrorContext::wal_file_read(
                file_path,
                offset,
                bytes_requested,
                bytes_read,
    ...
    pub fn wal_parse_error(
        ...
        RecoveryError::new(RecoveryErrorKind::WalFile, message)
            .with_context(ScannerErrorContext::wal_file_parse(
                file_path,
                offset,
                record_type,
                expected_format,
    ...
    pub fn wal_header_error(
        ...
        RecoveryError::new(RecoveryErrorKind::WalFile, message)
            .with_context(ScannerErrorContext::wal_header_validation(
                file_path,
                0,
                expected_version,
                actual_version,
    ...
    pub fn index_scan_error(
        ...
        RecoveryError::new(RecoveryErrorKind::WalFile, message)
            .with_context(ScannerErrorContext::wal_index_scan(
                file_path,
                index_offset,
                entry_count,
                failed_at_entry,
    ...
    pub fn sequence_scan_error(
        ...
        RecoveryError::new(RecoveryErrorKind::WalFile, message)
            .with_context(ScannerErrorContext::wal_sequence_scan(
                file_path,
                start_lsn,
                end_lsn,
                current_lsn,
    ...
    pub fn permission_error(
        ...
        let message = format!(
            "Permission denied for {} on file {}: {}",
            operation, file_path, required_permissions
        );

        RecoveryError::new(RecoveryErrorKind::Io, message)
            .with_context(ScannerErrorContext::file_system_access(
                operation,
                file_path,
                Some(required_permissions),
                None,
            ))
    ...
    pub fn disk_space_error(
        ...
        context
            .metadata
            .insert("required_space".to_string(), required_space.to_string());

        if let Some(space) = available_space {
            context
                .metadata
                .insert("available_space".to_string(), space.to_string());
        }
    ...
    pub fn format_detection_error(
        ...
        RecoveryError::new(RecoveryErrorKind::WalFile, message)
            .with_context(ScannerErrorContext::format_detection(
                file_path,
                magic_bytes,
                0,
                suspected_format,
    ...
    pub fn buffer_allocation_error(
        ...
        RecoveryError::new(RecoveryErrorKind::Resource, message)
            .with_context(ScannerErrorContext::buffer_management(
                operation,
                buffer_size,
                required_size,
                true,
...
impl ScannerErrorExt for RecoveryError {
    ...
    fn with_scanner_context(
        ...
        if let Some(off) = offset {
            context
                .metadata
                .insert("offset".to_string(), off.to_string());
        }
    ...
    fn with_file_metadata(
        ...
        if let Some(time) = modification_time {
            context
                .metadata
                .insert("modification_time".to_string(), format!("{:?}", time));
        }
...
mod tests {
    ...
    fn test_scanner_error_context_wal_read() {
        ...
        assert_eq!(
            context.metadata.get("bytes_requested"),
            Some(&"512".to_string())
        );
    ...
    fn test_scanner_error_context_wal_parse() {
        ...
        assert_eq!(
            context.metadata.get("data_preview"),
    ...
    fn test_scanner_error_context_wal_sequence() {
        let context =
            ScannerErrorContext::wal_sequence_scan("/test/wal.db", 1000, 2000, Some(1500), 250);
        ...
        assert_eq!(
            context.metadata.get("current_lsn"),
            Some(&"1500".to_string())
        );
    }
    ...
    fn test_scanner_error_factory_wal_read() {
        let error = ScannerErrorFactory::wal_read_error(
            "/test/wal.db",
            1024,
            512,
            Some(256),
    ...
    fn test_scanner_error_factory_wal_parse() {
        let error = ScannerErrorFactory::wal_parse_error(
            "/test/wal.db",
            2048,
            "EdgeRecord",
            "V2.0",
    ...
    fn test_scanner_error_factory_permission() {
        ...
        assert_eq!(error.kind, RecoveryErrorKind::Io);
        assert_eq!(
            error.context.metadata.get("operation"),
            Some(&"read".to_string())
        );
        assert_eq!(
            error.context.metadata.get("permissions_required"),
            Some(&"r".to_string())
        );
    ...
    fn test_scanner_error_factory_disk_space() {
        ...
        assert_eq!(
            error.context.metadata.get("required_space"),
            Some(&"1024".to_string())
        );
        assert_eq!(
            error.context.metadata.get("available_space"),
            Some(&"512".to_string())
        );
    ...
    fn test_scanner_error_factory_buffer_allocation() {
        ...
        assert_eq!(
            error.context.metadata.get("required_size"),
            Some(&"2048".to_string())
        );
        assert_eq!(
            error.context.metadata.get("buffer_size"),
            Some(&"1024".to_string())
        );
    }
    ...
    fn test_scanner_error_extension() {
        ...
        assert_eq!(
            scanner_error.context.metadata.get("offset"),
            Some(&"1024".to_string())
        );
        assert_eq!(scanner_error.context.records_processed, Some(512));
        assert_eq!(
            scanner_error.context.recovery_progress_percentage,
            Some(50.0)
        );
        assert_eq!(
            scanner_error.context.metadata.get("total_items"),
            Some(&"1024".to_string())
        );
        assert_eq!(
            scanner_error.context.metadata.get("file_size"),
            Some(&"2048".to_string())
        );
    }
    ...
    fn test_scanner_error_factory_initialization() {
        let error = ScannerErrorFactory::initialization_error(
            "Failed to init",
            "Scanner",
            Some("/test/wal.db"),
        );
