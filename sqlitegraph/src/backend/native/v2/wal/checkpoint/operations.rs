...
use crate::backend::native::v2::wal::checkpoint::core::{CheckpointProgress, CheckpointState, DirtyBlockTracker};
use crate::backend::native::v2::wal::{V2WALConfig, V2WALRecord, V2WALReader};
...
impl CheckpointExecutor {
    ...
    pub fn new(config: V2WALConfig) -> CheckpointResult<Self> {
        ...
        let v2_integrator = V2GraphIntegrator::new(v2_graph_path)
            .map_err(|e| CheckpointError::v2_integration(format!(
                "Failed to create V2 Graph Integrator: {}", e
            )))?;
    ...
    pub fn execute_incremental_checkpoint(
        ...
        self.write_checkpoint_header((start_lsn, end_lsn), checkpoint_start, dirty_block_offsets.len() as u64)?;
        ...
        for (i, (lsn, record)) in records.into_iter().enumerate() {
            ...
            {
                let mut v2_integrator = self.v2_integrator.lock()
                    .map_err(|e| CheckpointError::state(format!("Failed to lock V2 integrator: {}", e)))?;

                v2_integrator.apply_record_to_v2_graph(&record, lsn)
                    .map_err(|e| CheckpointError::v2_integration(format!(
                        "Failed to apply WAL record to V2 graph file: {}", e
                    )))?;
            }
    ...
    fn read_wal_records(&self, start_lsn: u64, end_lsn: u64) -> CheckpointResult<Vec<(u64, V2WALRecord)>> {
        let mut reader = V2WALReader::open(&self.config.wal_path)
    ...
    fn write_checkpoint_header(
        ...
        let mut checkpoint_file = self.checkpoint_file.lock()
            .map_err(|e| CheckpointError::state(format!("Failed to lock checkpoint file: {}", e)))?;

        // Write checkpoint magic number
        checkpoint_file.write_all(CHECKPOINT_MAGIC)
            .map_err(|e| CheckpointError::io(format!("Failed to write checkpoint magic: {}", e)))?;

        // Write checkpoint version
        checkpoint_file.write_all(&CHECKPOINT_VERSION.to_le_bytes())
            .map_err(|e| CheckpointError::io(format!("Failed to write checkpoint version: {}", e)))?;

        // Write LSN range
        checkpoint_file.write_all(&lsn_range.0.to_le_bytes())
            .map_err(|e| CheckpointError::io(format!("Failed to write checkpoint start LSN: {}", e)))?;

        checkpoint_file.write_all(&lsn_range.1.to_le_bytes())
            .map_err(|e| CheckpointError::io(format!("Failed to write checkpoint end LSN: {}", e)))?;

        // Write timestamp
        checkpoint_file.write_all(&timestamp.to_le_bytes())
            .map_err(|e| CheckpointError::io(format!("Failed to write checkpoint timestamp: {}", e)))?;

        // Write block count
        checkpoint_file.write_all(&block_count.to_le_bytes())
            .map_err(|e| CheckpointError::io(format!("Failed to write checkpoint block count: {}", e)))?;
    ...
    fn write_v2_metadata(&self, writer: &mut BufWriter<File>) -> CheckpointResult<()> {
        ...
        writer.write_all(&v2_version.to_le_bytes())
            .map_err(|e| CheckpointError::io(format!("Failed to write V2 version: {}", e)))?;

        // Write V2-specific configuration
        writer.write_all(&v2::V2_GRAPH_BLOCK_SIZE.to_le_bytes())
            .map_err(|e| CheckpointError::io(format!("Failed to write V2 block size: {}", e)))?;

        writer.write_all(&v2::V2_CLUSTER_ALIGNMENT.to_le_bytes())
            .map_err(|e| CheckpointError::io(format!("Failed to write V2 cluster alignment: {}", e)))?;

        // Write metadata length placeholder
        let metadata_length_pos = writer.stream_position()
            .map_err(|e| CheckpointError::io(format!("Failed to get metadata length position: {}", e)))?;
        writer.write_all(&0u32.to_le_bytes())
            .map_err(|e| CheckpointError::io(format!("Failed to write metadata length placeholder: {}", e)))?;
        ...
        let metadata_end = writer.stream_position()
            .map_err(|e| CheckpointError::io(format!("Failed to get metadata end position: {}", e)))?;
        let metadata_length = (metadata_end - metadata_start - 4) as u32;

        // Seek back and write actual metadata length
        writer.seek(SeekFrom::Start(metadata_length_pos))
            .map_err(|e| CheckpointError::io(format!("Failed to seek to metadata length: {}", e)))?;
        writer.write_all(&metadata_length.to_le_bytes())
            .map_err(|e| CheckpointError::io(format!("Failed to write metadata length: {}", e)))?;
        writer.seek(SeekFrom::Start(metadata_end))
            .map_err(|e| CheckpointError::io(format!("Failed to seek back to metadata end: {}", e)))?;
    ...
    fn write_checkpoint_progress(&self, progress: &CheckpointProgress) -> CheckpointResult<()> {
        ...
        checkpoint_file.flush()
            .map_err(|e| CheckpointError::io(format!("Failed to flush checkpoint progress: {}", e)))?;
    ...
    fn write_checkpoint_completion(&self, progress: &CheckpointProgress) -> CheckpointResult<()> {
        ...
        checkpoint_file.write_all(COMPLETION_MAGIC)
            .map_err(|e| CheckpointError::io(format!("Failed to write completion magic: {}", e)))?;

        // Write total records processed
        checkpoint_file.write_all(&progress.total_records.to_le_bytes())
        ...
        let completion_time = SystemTime::now()
            ...
            .as_secs();
        checkpoint_file.write_all(&completion_time.to_le_bytes())
...
impl V2GraphIntegrator {
    ...
    pub fn new(graph_file_path: PathBuf) -> CheckpointResult<Self> {
        ...
        let graph_file_ptr = unsafe { std::mem::transmute::<&mut GraphFile, &'static mut GraphFile>(&mut graph_file) };
    ...
    pub fn apply_record_to_v2_graph(&mut self, record: &V2WALRecord, lsn: u64) -> CheckpointResult<()> {
        match record {
            ...
            V2WALRecord::NodeDelete { node_id, slot_offset, old_data: _ } => {
            ...
            V2WALRecord::EdgeInsert { cluster_key, edge_record, insertion_point } => {
                ...
                self.apply_edge_insert(&typed_cluster_key, &edge_bytes, (*insertion_point).into(), lsn)
            }

            V2WALRecord::EdgeUpdate { cluster_key, old_edge: _, new_edge, position } => {
                let typed_cluster_key = (cluster_key.0 as u64, cluster_key.1 as u64);
                let edge_bytes = new_edge.as_bytes();
                self.apply_edge_update(&typed_cluster_key, &edge_bytes, *position as u64, lsn)
            }

            V2WALRecord::EdgeDelete { cluster_key, old_edge: _, position } => {
                let typed_cluster_key = (cluster_key.0 as u64, cluster_key.1 as u64);
            ...
            V2WALRecord::FreeSpaceDeallocate { block_offset, block_size: _, block_type: _ } => {
    ...
    fn apply_node_insert(&mut self, node_id: u64, slot_offset: u64, node_data: &[u8], _lsn: u64) -> CheckpointResult<()> {
        // Validate input parameters
        if node_data.is_empty() {
            return Err(CheckpointError::validation("Node data cannot be empty".to_string()));
        }

        // Convert node_id to i64 for V2 backend
        let node_id_i64 = node_id as i64;
        if node_id_i64 <= 0 {
            return Err(CheckpointError::validation("Node ID must be positive".to_string()));
        }

        // Deserialize NodeRecordV2 from WAL data
        let node_record = NodeRecordV2::deserialize(node_data)
            .map_err(|e| CheckpointError::v2_integration(format!(
                "Failed to deserialize NodeRecordV2: {}", e
            )))?;

        // Validate node record
        node_record.validate()
            .map_err(|e| CheckpointError::validation(format!(
                "Invalid NodeRecordV2: {}", e
            )))?;
        ...
        node_store.write_node_v2(&node_record)
            .map_err(|e| CheckpointError::v2_integration(format!(
                "Failed to write NodeRecordV2 to graph file: {}", e
            )))?;
        ...
        graph_file.flush()
            .map_err(|e| CheckpointError::io(format!(
                "Failed to sync graph file after node insert: {}", e
            )))?;
    ...
    fn apply_edge_insert(
        ...
        let edge_record_data: crate::backend::native::EdgeRecord = serde_json::from_slice(edge_record)
            .map_err(|e| CheckpointError::v2_integration(format!(
                "Failed to deserialize edge record: {}", e
            )))?;

        // Create edge cluster using real V2 EdgeCluster operations
        let mut string_table = self.string_table.lock()
        ...
        let edge_cluster = EdgeCluster::create_from_edges(
            ...
            "Failed to create edge cluster: {}", e
        )))?;

        // Validate cluster integrity
        edge_cluster.validate()
            .map_err(|e| CheckpointError::validation(format!(
                "Invalid edge cluster: {}", e
            )))?;
        ...
        let cluster_offset = {
            let mut free_space = self.free_space_manager.lock()
                .map_err(|e| CheckpointError::state(format!("Failed to lock free space manager: {}", e)))?;

            let offset = free_space.allocate(serialized_cluster.len() as u32)
                .map_err(|e| CheckpointError::v2_integration(format!(
                    "Failed to allocate cluster space: {:?}", e
                )))?;
        ...
        let mut node_record = node_store.read_node_v2(from_node_id)
            .map_err(|e| CheckpointError::v2_integration(format!(
                "Failed to read node record for cluster update: {}", e
            )))?;
        ...
        node_store.write_node_v2(&node_record)
            .map_err(|e| CheckpointError::v2_integration(format!(
                "Failed to update node record with cluster reference: {}", e
            )))?;
        ...
        graph_file.flush()
            .map_err(|e| CheckpointError::io(format!(
                "Failed to sync graph file after edge insert: {}", e
            )))?;
    ...
    fn apply_cluster_create(
        ...
        if cluster_size > MAX_V2_CLUSTER_SIZE {
            return Err(CheckpointError::validation("Cluster size exceeds maximum".to_string()));
        }
        ...
        let cluster_direction = match direction {
            ...
            _ => return Err(CheckpointError::validation("Invalid cluster direction".to_string())),
        };

        // Validate edge data
        if edge_data.is_empty() {
            return Err(CheckpointError::validation("Edge data cannot be empty for cluster creation".to_string()));
        }

        // Create cluster from edge data
        let edge_cluster = EdgeCluster::deserialize(edge_data)
            .map_err(|e| CheckpointError::v2_integration(format!(
                "Failed to deserialize edge cluster data: {}", e
            )))?;

        // Validate cluster integrity
        edge_cluster.validate()
            .map_err(|e| CheckpointError::validation(format!(
                "Invalid edge cluster data: {}", e
            )))?;
        ...
        let mut node_record = node_store.read_node_v2(node_id_i64)
            .map_err(|e| CheckpointError::v2_integration(format!(
                "Failed to read node record for cluster creation: {}", e
            )))?;
        ...
        node_store.write_node_v2(&node_record)
            .map_err(|e| CheckpointError::v2_integration(format!(
                "Failed to update node record with cluster metadata: {}", e
            )))?;

        // Update free space manager to mark cluster region as used
        let mut free_space_manager = self.free_space_manager.lock()
            .map_err(|e| CheckpointError::state(format!("Failed to lock free space manager: {}", e)))?;
        ...
        graph_file.flush()
            .map_err(|e| CheckpointError::io(format!(
                "Failed to sync graph file after cluster creation: {}", e
            )))?;
    ...
    fn apply_string_table_insert(&mut self, string_id: u64, string_data: &[u8], _lsn: u64) -> CheckpointResult<()> {
        // Validate input parameters
        if string_data.is_empty() {
            return Err(CheckpointError::validation("String data cannot be empty".to_string()));
        }

        if string_id == 0 {
            return Err(CheckpointError::validation("String ID cannot be zero".to_string()));
        }

        // Convert string data to UTF-8 string
        let string_value = std::str::from_utf8(string_data)
            .map_err(|_| CheckpointError::validation("String data contains invalid UTF-8".to_string()))?;
        ...
        // Add string to table
        let assigned_offset = string_table.get_or_add_offset(string_value)
            .map_err(|e| CheckpointError::v2_integration(format!(
                "Failed to add string to string table: {}", e
            )))?;
        ...
        if assigned_offset as u64 != string_id {
            return Err(CheckpointError::v2_integration(format!(
                "String ID mismatch: expected {}, got {}", string_id, assigned_offset
            )));
        }
    ...
    fn apply_free_space_insert(&mut self, region_offset: u64, region_size: u64, _lsn: u64) -> CheckpointResult<()> {
        // Validate input parameters
        if region_size == 0 {
            return Err(CheckpointError::validation("Free space region size cannot be zero".to_string()));
        }

        if region_size < MIN_FREE_SPACE_REGION_SIZE {
            return Err(CheckpointError::validation("Free space region too small".to_string()));
        }

        if region_size > MAX_FREE_SPACE_REGION_SIZE {
            return Err(CheckpointError::validation("Free space region too large".to_string()));
        }
    ...
    fn apply_node_update(&mut self, node_id: u64, _slot_offset: u64, node_data: &[u8], _lsn: u64) -> CheckpointResult<()> {
        let node_id_i64 = node_id as i64;
        if node_id_i64 <= 0 {
            return Err(CheckpointError::validation("Node ID must be positive".to_string()));
        }

        let updated_node = NodeRecordV2::deserialize(node_data)
            .map_err(|e| CheckpointError::v2_integration(format!(
                "Failed to deserialize updated NodeRecordV2: {}", e
            )))?;
        ...
        node_store.write_node_v2(&updated_node)
            .map_err(|e| CheckpointError::v2_integration(format!(
                "Failed to update NodeRecordV2: {}", e
            )))?;

        Ok(())
    }

    fn apply_node_delete(&mut self, node_id: u64, _slot_offset: u64, _lsn: u64) -> CheckpointResult<()> {
        let node_id_i64 = node_id as i64;
        if node_id_i64 <= 0 {
            return Err(CheckpointError::validation("Node ID must be positive".to_string()));
        }
        ...
        let existing_node = node_store.read_node_v2(node_id_i64)
            .map_err(|e| CheckpointError::v2_integration(format!(
                "Failed to read node for deletion: {}", e
            )))?;
        ...
        node_store.write_node_v2(&deleted_node)
            .map_err(|e| CheckpointError::v2_integration(format!(
                "Failed to mark node as deleted: {}", e
            )))?;

        Ok(())
    }

    fn apply_edge_update(&mut self, _cluster_key: &(u64, u64), _edge_record: &[u8], _update_point: u64, _lsn: u64) -> CheckpointResult<()> {
    ...
    fn apply_edge_delete(&mut self, _cluster_key: &(u64, u64), _deletion_point: u64, _lsn: u64) -> CheckpointResult<()> {
    ...
    fn apply_cluster_update(&mut self, node_id: u64, direction: u8, cluster_offset: u64, cluster_size: u64, edge_data: &[u8], _lsn: u64) -> CheckpointResult<()> {
        // Use same logic as cluster create for updating cluster metadata
        self.apply_cluster_create(node_id, direction, cluster_offset, cluster_size, edge_data, 0)
    }

    fn apply_cluster_delete(&mut self, node_id: u64, direction: u8, cluster_offset: u64, _lsn: u64) -> CheckpointResult<()> {
        let node_id_i64 = node_id as i64;
        if node_id_i64 <= 0 {
            return Err(CheckpointError::validation("Node ID must be positive".to_string()));
        }
        ...
        let mut node_record = node_store.read_node_v2(node_id_i64)
            .map_err(|e| CheckpointError::v2_integration(format!(
                "Failed to read node for cluster deletion: {}", e
            )))?;
        ...
        node_store.write_node_v2(&node_record)
            .map_err(|e| CheckpointError::v2_integration(format!(
                "Failed to update node record after cluster deletion: {}", e
            )))?;
...
impl BlockFlusher {
    ...
    pub fn flush_dirty_block(&self, block_offset: u64) -> CheckpointResult<()> {
        ...
        if block_offset % V2_GRAPH_BLOCK_SIZE != 0 {
            return Err(CheckpointError::validation(format!(
                "Block offset {} not aligned to V2 block size {}", block_offset, V2_GRAPH_BLOCK_SIZE
            )));
        }

        // Open V2 graph file for real block flushing
        let mut graph_file = GraphFile::open(&self.v2_graph_path)
            .map_err(|e| CheckpointError::v2_integration(format!(
        ...
        let file_size = graph_file.file_size()
            .map_err(|e| CheckpointError::v2_integration(format!(
                "Failed to get V2 graph file size: {}", e
            )))?;

        if block_offset + V2_GRAPH_BLOCK_SIZE > file_size {
            return Err(CheckpointError::validation(format!(
                "Block offset {} exceeds V2 graph file size {}", block_offset, file_size
            )));
        }
        ...
        graph_file.flush()
            .map_err(|e| CheckpointError::io(format!(
                "Failed to sync V2 graph file during block flush: {}", e
            )))?;
    ...
    pub fn flush_dirty_blocks(&self, block_offsets: &[u64]) -> CheckpointResult<()> {
        ...
        graph_file.flush()
            .map_err(|e| CheckpointError::io(format!(
                "Failed to sync V2 graph file during batch block flush: {}", e
            )))?;
...
mod tests {
    ...
    fn test_v2_graph_integrator_creation() -> CheckpointResult<()> {
        ...
        assert!(integrator.is_ok(), "V2GraphIntegrator creation should succeed");
        Ok(())
    }
    ...
    fn test_v2_graph_integrator_node_insert() -> CheckpointResult<()> {
        ...
        // Create a minimal V2 graph file for testing
        let graph_file = GraphFile::create(&v2_graph_path)
            .map_err(|e| CheckpointError::v2_integration(format!("Failed to create test graph file: {}", e)))?;
        ...
        assert!(result.is_ok(), "Node insert should succeed with real V2 backend");
        Ok(())
    }
    ...
    fn test_block_flusher_creation() {
        ...
        assert_eq!(flusher.v2_graph_path(), std::path::Path::new("/tmp/test.v2"));
    }
    ...
    fn test_block_flusher_invalid_offset() -> CheckpointResult<()> {
        ...
        assert!(result.is_err(), "Non-aligned block offset should fail validation");
        Ok(())
    }
    ...
    fn test_block_flusher_with_real_v2_file() -> CheckpointResult<()> {
        ...
        assert!(result.is_err(), "Block flush should fail for file too small for full block (expected validation behavior)");
