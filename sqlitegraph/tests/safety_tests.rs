...
use sqlitegraph::{
    ...
    safety::{
        run_deep_safety_checks, run_integrity_sweep, run_safety_checks, run_strict_safety_checks,
...
#[test]
fn integrity_sweep_detects_invalid_entity_json() {
    let ctx = graph_context("sweep_invalid");
    let conn = raw_conn(&ctx.path);
    conn.execute(
        "UPDATE graph_entities SET data='not-json' WHERE id=?1",
        params![ctx.root],
    )
    .unwrap();
    let issues = run_integrity_sweep(&ctx.graph).unwrap();
    assert!(issues.iter().any(|msg| msg.contains("invalid JSON")));
}

#[test]
fn integrity_sweep_clean_graph_is_empty() {
    let ctx = graph_context("sweep_clean");
    let issues = run_integrity_sweep(&ctx.graph).unwrap();
    assert!(issues.is_empty());
}

#[test]
fn deep_scan_reports_integrity_fields() {
    let ctx = graph_context("deep");
    let report = run_deep_safety_checks(&ctx.graph).unwrap();
    assert_eq!(report.integrity_errors, 0);
    assert!(report.integrity_messages.is_empty());
}
